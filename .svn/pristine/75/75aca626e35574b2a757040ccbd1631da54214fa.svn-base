...
class ViewLegalInformation extends HtmlResponse
{
   	function TemplateModule()
   	{
      	$this->SetTemplateBasedir(Configuration::Instance()->GetValue('application','docroot').'module/'.GtfwDispt()->mModule.'/template');
      	$this->SetTemplateFile('view_legalinformation.html');
   	}
   	...
   	{	
		...
		$total = $ObjLegalInformation->countLegalInformation($filter);
		$data = $ObjLegalInformation->getLegalInformation($filter);
   	...
   	function ParseTemplate($rdata = NULL)
   	{
        ...
        if (!empty($data) AND count($data)>0) {
            ...
            foreach ($data as $val) {       
                $val['no'] = $no;
                $val['url_detail'] = GtfwDispt()->GetUrl('cms.legal.information', 'detailLegalInformation', 'view', 'html').'&id='.$val['id'];
                
                $this->mrTemplate->clearTemplate('button_update');
                $this->mrTemplate->addVar('button_update', 'URL', GtfwDispt()->GetUrl('cms.legal.information', 'updateLegalInformation', 'view', 'html').'&id='.$val['id']);
                
                $this->mrTemplate->clearTemplate('button_delete');
                $this->mrTemplate->addVar('button_delete', 'NAME', $val['title']);
                $this->mrTemplate->addVar('button_delete', 'URL', GtfwDispt()->GetUrl('cms.legal.information', 'deleteLegalInformation', 'do', 'json').'&id='.$val['id']);
        ...
        $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('cms.legal.information', 'addLegalInformation', 'view', 'html'));
   	}
}
?>