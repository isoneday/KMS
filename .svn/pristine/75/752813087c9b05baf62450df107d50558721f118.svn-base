 ...
 * @author Prima Noor 
 */
class ViewSmsTemplate extends HtmlResponse {

    function TemplateModule() {
        $this->SetTemplateBasedir(Configuration::Instance()->GetValue('application', 'docroot') . 'module/' . GtfwDispt()->mModule . '/template');
        $this->SetTemplateFile('view_smstemplate.html');
    }
    ...
    function ProcessRequest() {
        ...
        $data = $ObjSmsTemplate->getSmsTemplate($filter);
        $total = $ObjSmsTemplate->countSmsTemplate();
    ...
    function ParseTemplate($rdata = NULL) {
        ...
        if (!empty($data) AND count($data) > 0) {
            ...
            foreach ($data as $val) {
                $val['no'] = $no;
                $val['url_detail'] = GtfwDispt()->GetUrl('comp.sms.template', 'detailSmsTemplate', 'view', 'html') . '&id=' . $val['id'];

                $this->mrTemplate->clearTemplate('button_update');
                $this->mrTemplate->addVar('button_update', 'URL', GtfwDispt()->GetUrl('comp.sms.template', 'updateSmsTemplate', 'view', 'html') . '&id=' . $val['id']);

                $this->mrTemplate->clearTemplate('button_delete');
                $this->mrTemplate->addVar('button_delete', 'NAME', $val['purpose']);
                $this->mrTemplate->addVar('button_delete', 'URL', GtfwDispt()->GetUrl('comp.sms.template', 'deleteSmsTemplate', 'do', 'json') . '&id=' . $val['id']);
        ...
        $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('comp.sms.template', 'addSmsTemplate', 'view', 'html'));
    }
