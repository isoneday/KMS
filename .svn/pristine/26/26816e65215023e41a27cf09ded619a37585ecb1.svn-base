...
class ViewComboPrivacyPolice extends HtmlResponse
{
    function TemplateModule()
    {
        $this->SetTemplateBasedir(Configuration::Instance()->GetValue('application', 'docroot') . 'module/cms.privacy.police/template');
        $this->SetTemplateFile('view_combo_privacypolice.html');
    }
    ...
    function ProcessRequest()
    {
        ...
        Messenger::Instance()->Send('cms.privacy.police', 'comboPrivacyPolice', 'view', 'html', $comboData, Messenger::UntilFetched);
        
        $allowAdd = Security::Authorization()->IsAllowedToAccess('cms.privacy.police', 'addPrivacyPolice', 'view', 'html');
        
        $list = $ObjPrivacyPolice->listPrivacyPolice();
    ...
    function ParseTemplate($rdata = null)
    {
        ...
        if (!empty($comboData['elmId'])) {
            ...
            if (!empty($comboData['showAdd']) and $allowAdd) {
                ...
                $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('cms.privacy.police', 'addPrivacyPolice', 'view', 'html').'&elmId='.$comboData['elmId']);
            }
        }
