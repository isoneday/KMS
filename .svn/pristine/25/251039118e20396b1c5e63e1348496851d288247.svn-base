...
class ViewComboCompanyProfile extends HtmlResponse
{
    function TemplateModule()
    {
        $this->SetTemplateBasedir(Configuration::Instance()->GetValue('application', 'docroot') . 'module/cms.company.profile/template');
        $this->SetTemplateFile('view_combo_companyprofile.html');
    }
    ...
    function ProcessRequest()
    {
        ...
        Messenger::Instance()->Send('cms.company.profile', 'comboCompanyProfile', 'view', 'html', $comboData, Messenger::UntilFetched);
        
        $allowAdd = Security::Authorization()->IsAllowedToAccess('cms.company.profile', 'addCompanyProfile', 'view', 'html');
        
        $list = $ObjCompanyProfile->listCompanyProfile();
    ...
    function ParseTemplate($rdata = null)
    {
        ...
        if (!empty($comboData['elmId'])) {
            ...
            if (!empty($comboData['showAdd']) and $allowAdd) {
                ...
                $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('cms.company.profile', 'addCompanyProfile', 'view', 'html').'&elmId='.$comboData['elmId']);
            }
        }
