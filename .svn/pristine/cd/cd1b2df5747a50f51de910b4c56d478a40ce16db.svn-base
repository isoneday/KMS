...
class ViewLanguage extends HtmlResponse
{
   	function TemplateModule()
   	{
      	$this->SetTemplateBasedir(Configuration::Instance()->GetValue('application','docroot').'module/'.GtfwDispt()->mModule.'/template');
      	$this->SetTemplateFile('view_language.html');
   	}
   	...
   	{	
		...
		$total = $ObjLanguage->countLanguage($filter);
		$data = $ObjLanguage->getLanguage($filter);
   	...
   	function ParseTemplate($rdata = NULL)
   	{
        ...
        if (!empty($data) AND count($data)>0) {
            ...
            foreach ($data as $val) {       
                $val['no'] = $no;
                $val['url_detail'] = GtfwDispt()->GetUrl('core.language', 'detailLanguage', 'view', 'html').'&id='.$val['code'];
                
                $this->mrTemplate->clearTemplate('button_update');
                $this->mrTemplate->addVar('button_update', 'URL', GtfwDispt()->GetUrl('core.language', 'updateLanguage', 'view', 'html').'&id='.$val['code']);
                
                $this->mrTemplate->clearTemplate('button_delete');
                $this->mrTemplate->addVar('button_delete', 'NAME', $val['name']);
                $this->mrTemplate->addVar('button_delete', 'URL', GtfwDispt()->GetUrl('core.language', 'deleteLanguage', 'do', 'json').'&id='.$val['code']);
        ...
        $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('core.language', 'addLanguage', 'view', 'html'));
   	}
}
?>