 ...
 * @author Prima Noor 
 */
class ViewCmsNews extends HtmlResponse {

    function TemplateModule() {
        $this->SetTemplateBasedir(Configuration::Instance()->GetValue('application', 'docroot') . 'module/' . GtfwDispt()->mModule . '/template');
        $this->SetTemplateFile('view_cmsnews.html');
    }
    ...
    function ProcessRequest() {
        ...
        $data = $ObjCmsNews->getCmsNews($filter);
        $total = $ObjCmsNews->countCmsNews();
    ...
    function ParseTemplate($rdata = NULL) {
        ...
        if (!empty($data) AND count($data) > 0) {
            ...
            foreach ($data as $val) {
                $val['no'] = $no;
                $val['url_detail'] = GtfwDispt()->GetUrl('cms.news', 'detailCmsNews', 'view', 'html') . '&id=' . $val['id'];
                ...
                $this->mrTemplate->addVar('button_update', 'URL', GtfwDispt()->GetUrl('cms.news', 'updateCmsNews', 'view', 'html') . '&id=' . $val['id']);

                $this->mrTemplate->clearTemplate('button_delete');
                $this->mrTemplate->addVar('button_delete', 'NAME', $val['title']);
                $this->mrTemplate->addVar('button_delete', 'URL', GtfwDispt()->GetUrl('cms.news', 'deleteCmsNews', 'do', 'json') . '&id=' . $val['id']);
        ...
        $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('cms.news', 'addCmsNews', 'view', 'html'));
    }
