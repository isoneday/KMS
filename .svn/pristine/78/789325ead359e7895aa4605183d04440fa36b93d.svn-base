 ...
 * @author Prima Noor 
 */
class ViewComboRefEmpStatus extends HtmlResponse {

    function TemplateModule() {
        $this->SetTemplateBasedir(Configuration::Instance()->GetValue('application', 'docroot') . 'module/emp.ref.employee.status/template');
        $this->SetTemplateFile('view_combo_status.html');
    }
    ...
    function ProcessRequest() {
        ...
        Messenger::Instance()->Send('emp.ref.employee.status', 'comboRefEmpStatus', 'view', 'html', $comboData, Messenger::UntilFetched);

        $allowAdd = Security::Authorization()->IsAllowedToAccess('emp.ref.employee.status', 'addStatus', 'view', 'html');

        $list = $ObjStatus->listStatus();
    ...
    function ParseTemplate($rdata = null) {
        ...
        if (!empty($comboData['elmId'])) {
            ...
            if (!empty($comboData['showAdd']) and $allowAdd) {
                ...
                $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('emp.ref.employee.status', 'addStatus', 'view', 'html') . '&elmId=' . $comboData['elmId']);
            }
        }
