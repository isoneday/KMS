...
class ViewComboSliderBanner extends HtmlResponse
{
    function TemplateModule()
    {
        $this->SetTemplateBasedir(Configuration::Instance()->GetValue('application', 'docroot') . 'module/cms.slider.banner/template');
        $this->SetTemplateFile('view_combo_sliderbanner.html');
    }
    ...
    function ProcessRequest()
    {
        ...
        Messenger::Instance()->Send('cms.slider.banner', 'comboSliderBanner', 'view', 'html', $comboData, Messenger::UntilFetched);
        
        $allowAdd = Security::Authorization()->IsAllowedToAccess('cms.slider.banner', 'addSliderBanner', 'view', 'html');
        
        $list = $ObjSliderBanner->listSliderBanner();
    ...
    function ParseTemplate($rdata = null)
    {
        ...
        if (!empty($comboData['elmId'])) {
            ...
            if (!empty($comboData['showAdd']) and $allowAdd) {
                ...
                $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('cms.slider.banner', 'addSliderBanner', 'view', 'html').'&elmId='.$comboData['elmId']);
            }
        }
