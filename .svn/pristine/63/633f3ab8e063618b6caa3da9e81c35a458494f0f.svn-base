...
class ViewComboLegalInformation extends HtmlResponse
{
    function TemplateModule()
    {
        $this->SetTemplateBasedir(Configuration::Instance()->GetValue('application', 'docroot') . 'module/cms.legal.information/template');
        $this->SetTemplateFile('view_combo_legalinformation.html');
    }
    ...
    function ProcessRequest()
    {
        ...
        Messenger::Instance()->Send('cms.legal.information', 'comboLegalInformation', 'view', 'html', $comboData, Messenger::UntilFetched);
        
        $allowAdd = Security::Authorization()->IsAllowedToAccess('cms.legal.information', 'addLegalInformation', 'view', 'html');
        
        $list = $ObjLegalInformation->listLegalInformation();
    ...
    function ParseTemplate($rdata = null)
    {
        ...
        if (!empty($comboData['elmId'])) {
            ...
            if (!empty($comboData['showAdd']) and $allowAdd) {
                ...
                $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('cms.legal.information', 'addLegalInformation', 'view', 'html').'&elmId='.$comboData['elmId']);
            }
        }
