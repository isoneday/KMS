...
class ViewType extends HtmlResponse {
    ...
    function ProcessRequest() {
		...
		$total = $ObjType->countType($filter);
    ...
    function ParseTemplate($rdata = NULL) {
		...
		$this->mrTemplate->addGlobalVar('lang_module', GtfwLangText('employee_type'));
        ...
        if (!empty($data) AND count($data) > 0) {
            ...
            foreach ($data as $val) {
                $val['no'] = $no;
                $val['url_detail'] = GtfwDispt()->GetUrl('emp.ref.employee.type', 'detailType', 'view', 'html') . '&id=' . Dispatcher::Instance()->Encrypt($val['id']);
                $val['status'] == 1 ? $val['status'] = GtfwLangText('active') : $val['status'] = GtfwLangText('not_active');
                $val['is_permanent'] == 1 ? $val['is_permanent'] = GtfwLangText('yes') : $val['is_permanent'] = GtfwLangText('no');

                $this->mrTemplate->clearTemplate('button_update');
                $this->mrTemplate->addVar('button_update', 'URL', GtfwDispt()->GetUrl('emp.ref.employee.type', 'updateType', 'view', 'html') . '&id=' . Dispatcher::Instance()->Encrypt($val['id']));

                $this->mrTemplate->clearTemplate('button_delete');
                $this->mrTemplate->addVar('button_delete', 'NAME', $val['name']);
                $this->mrTemplate->addVar('button_delete', 'URL', GtfwDispt()->GetUrl('emp.ref.employee.type', 'deleteType', 'do', 'json') . '&id=' . Dispatcher::Instance()->Encrypt($val['id']));
        ...
        $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('emp.ref.employee.type', 'addType', 'view', 'html'));
    }
