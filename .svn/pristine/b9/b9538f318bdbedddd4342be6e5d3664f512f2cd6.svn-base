 ...
 * @author Prima Noor 
 */
class ViewInputNotificationTemplate extends HtmlResponse {

    function TemplateModule() {
        $this->SetTemplateBasedir(Configuration::Instance()->GetValue('application', 'docroot') . 'module/' . GtfwDispt()->mModule . '/template');
        $this->SetTemplateFile('view_input_notificationtemplate.html');
    }
    ...
    function ProcessRequest() {
        ...
        if (!empty($post)) {
            ...
            $input = $ObjNotificationTemplate->getDetailNotificationTemplate($id);
    ...
    function ParseTemplate($rdata = NULL) {
        ...
        $this->mrTemplate->addVar('content', 'URL_BACK', GtfwDispt()->GetUrl('comp.notification.template', 'NotificationTemplate', 'view', 'html') . '&display');
        $this->mrTemplate->addVar('content', 'URL', GtfwDispt()->GetUrl('comp.notification.template', (empty($id) ? 'add' : 'update') . 'NotificationTemplate', 'do', 'json') . '&elmId=' . $elmId);
    }
