...
class ViewComboCmsFaq extends HtmlResponse
{
    function TemplateModule()
    {
        $this->SetTemplateBasedir(Configuration::Instance()->GetValue('application', 'docroot') . 'module/cms.faq/template');
        $this->SetTemplateFile('view_combo_cmsfaq.html');
    }
    ...
    function ProcessRequest()
    {
        ...
        Messenger::Instance()->Send('cms.faq', 'comboCmsFaq', 'view', 'html', $comboData, Messenger::UntilFetched);
        
        $allowAdd = Security::Authorization()->IsAllowedToAccess('cms.faq', 'addCmsFaq', 'view', 'html');
        
        $list = $ObjCmsFaq->listCmsFaq();
    ...
    function ParseTemplate($rdata = null)
    {
        ...
        if (!empty($comboData['elmId'])) {
            ...
            if (!empty($comboData['showAdd']) and $allowAdd) {
                ...
                $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('cms.faq', 'addCmsFaq', 'view', 'html').'&elmId='.$comboData['elmId']);
            }
        }
