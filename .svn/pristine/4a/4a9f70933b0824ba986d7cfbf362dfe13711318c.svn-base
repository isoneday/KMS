 ...
 * @author Prima Noor 
 */
class ViewUnit extends HtmlResponse {

    function TemplateModule() {
        $this->SetTemplateBasedir(Configuration::Instance()->GetValue('application', 'docroot') . 'module/' . GtfwDispt()->mModule . '/template');
        $this->SetTemplateFile('view_unit.html');
    }
    ...
    function ProcessRequest() {
        ...
        Messenger::Instance()->SendToComponent('core.unit', 'comboUnit', 'view', 'html', 'parent_id', array(
            'dataId' => (!empty($filter['parent_id']) ? $filter['parent_id'] : null), 
            'elmId' => 'parent_id', 
            'first' => 'select', 
            'showAdd' => false,
            'name' => 'parent_id',
        ...
        $total = $ObjUnit->countUnit($filter);
        $data = $ObjUnit->getUnit($filter);
    ...
    function ParseTemplate($rdata = NULL) {
        ...
        if (!empty($data) AND count($data) > 0) {
            ...
            foreach ($data as $val) {
                $val['no'] = $no;
                $val['url_detail'] = GtfwDispt()->GetUrl('core.unit', 'detailUnit', 'view', 'html') . '&id=' . $val['id'];

                $this->mrTemplate->clearTemplate('button_update');
                $this->mrTemplate->addVar('button_update', 'URL', GtfwDispt()->GetUrl('core.unit', 'updateUnit', 'view', 'html') . '&id=' . $val['id']);

                $this->mrTemplate->clearTemplate('button_delete');
                $this->mrTemplate->addVar('button_delete', 'NAME', $val['name']);
                $this->mrTemplate->addVar('button_delete', 'URL', GtfwDispt()->GetUrl('core.unit', 'deleteUnit', 'do', 'json') . '&id=' . $val['id']);
        ...
        $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('core.unit', 'addUnit', 'view', 'html'));
    }
