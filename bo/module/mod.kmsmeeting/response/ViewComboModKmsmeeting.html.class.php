...
class ViewComboModKmsmeeting extends HtmlResponse
{
    function TemplateModule()
    {
        $this->SetTemplateBasedir(Configuration::Instance()->GetValue('application', 'docroot') . 'module/mod.kmsmeeting/template');
        $this->SetTemplateFile('view_combo_modkmsmeeting.html');
    }
    ...
    function ProcessRequest()
    {
        ...
        Messenger::Instance()->Send('mod.kmsmeeting', 'comboModKmsmeeting', 'view', 'html', $comboData, Messenger::UntilFetched);
        
        $allowAdd = Security::Authorization()->IsAllowedToAccess('mod.kmsmeeting', 'addModKmsmeeting', 'view', 'html');
        
        $list = $ObjModKmsmeeting->listModKmsmeeting();
    ...
    function ParseTemplate($rdata = null)
    {
        ...
        if (!empty($comboData['elmId'])) {
            ...
            if (!empty($comboData['showAdd']) and $allowAdd) {
                ...
                $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('mod.kmsmeeting', 'addModKmsmeeting', 'view', 'html').'&elmId='.$comboData['elmId']);
            }
        }
