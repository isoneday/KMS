...
class ViewComboModKmsproblem extends HtmlResponse
{
    function TemplateModule()
    {
        $this->SetTemplateBasedir(Configuration::Instance()->GetValue('application', 'docroot') . 'module/mod.kmsproblem/template');
        $this->SetTemplateFile('view_combo_modkmsproblem.html');
    }
    ...
    function ProcessRequest()
    {
        ...
        Messenger::Instance()->Send('mod.kmsproblem', 'comboModKmsproblem', 'view', 'html', $comboData, Messenger::UntilFetched);
        
        $allowAdd = Security::Authorization()->IsAllowedToAccess('mod.kmsproblem', 'addModKmsproblem', 'view', 'html');
        
        $list = $ObjModKmsproblem->listModKmsproblem();
    ...
    function ParseTemplate($rdata = null)
    {
        ...
        if (!empty($comboData['elmId'])) {
            ...
            if (!empty($comboData['showAdd']) and $allowAdd) {
                ...
                $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('mod.kmsproblem', 'addModKmsproblem', 'view', 'html').'&elmId='.$comboData['elmId']);
            }
        }
