 ...
 * @author Prima Noor 
 */
class ViewCoreCompany extends HtmlResponse {

    function TemplateModule() {
        $this->SetTemplateBasedir(Configuration::Instance()->GetValue('application', 'docroot') . 'module/' . GtfwDispt()->mModule . '/template');
        $this->SetTemplateFile('view_corecompany.html');
    }
    ...
    function ProcessRequest() {
        ...
        $data = $ObjCoreCompany->getCoreCompany($filter);
        $total = $ObjCoreCompany->countCoreCompany();
    ...
    function ParseTemplate($rdata = NULL) {
        ...
        if (!empty($data) AND count($data) > 0) {
            ...
            foreach ($data as $val) {
                $val['no'] = $no;
                $val['url_detail'] = GtfwDispt()->GetUrl('core.company', 'detailCoreCompany', 'view', 'html') . '&id=' . $val['id'];

                $this->mrTemplate->clearTemplate('button_update');
                $this->mrTemplate->addVar('button_update', 'URL', GtfwDispt()->GetUrl('core.company', 'updateCoreCompany', 'view', 'html') . '&id=' . $val['id']);

                $this->mrTemplate->clearTemplate('button_delete');
                $this->mrTemplate->addVar('button_delete', 'NAME', $val['name']);
                $this->mrTemplate->addVar('button_delete', 'URL', GtfwDispt()->GetUrl('core.company', 'deleteCoreCompany', 'do', 'json') . '&id=' . $val['id']);
        ...
        $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('core.company', 'addCoreCompany', 'view', 'html'));
    }
