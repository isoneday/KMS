 ...
 * @author Prima Noor 
 */
class ViewComboCompany extends HtmlResponse {

    function TemplateModule() {
        $this->SetTemplateBasedir(Configuration::Instance()->GetValue('application', 'docroot') . 'module/core.company/template');
        $this->SetTemplateFile('view_combo_company.html');
    }
    ...
    function ProcessRequest() {
        ...
        Messenger::Instance()->Send('core.company', 'comboFinCoa', 'view', 'html', $comboData, Messenger::UntilFetched);

        $allowAdd = Security::Authorization()->IsAllowedToAccess('fin.coa', 'addCoreCompany', 'view', 'html');
        ...
        if ($type == 'taf') {
            $list = $ObjCoreCompany->listCompanyToTaf();
        } else {
            $list = $ObjCoreCompany->listCompanyTransaction();
        }
    ...
    function ParseTemplate($rdata = null) {
        ...
        if (!empty($comboData['elmId'])) {
            ...
            if (!empty($comboData['showAdd']) and $allowAdd) {
                ...
                $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('fin.ref.bg.budget', 'addBgBudget', 'view', 'html') . '&elmId=' . $comboData['elmId']);
            }
        }
