...
class ViewSetting extends HtmlResponse
{
   	...
   	{	
		...
		$data = $ObjSetting->getSetting($filter);        
		$total = $ObjSetting->countSetting();
   	...
   	function ParseTemplate($rdata = NULL)
   	{
        ...
        if (!empty($data) AND count($data)>0) {
            ...
            foreach ($data as $val) {       
                $val['no'] = $no;
                $val['url_detail'] = GtfwDispt()->GetUrl('core.setting', 'detailSetting', 'view', 'html').'&id='.Dispatcher::Instance()->Encrypt ($val['id']);
                
                $this->mrTemplate->clearTemplate('button_update');
                $this->mrTemplate->addVar('button_update', 'URL', GtfwDispt()->GetUrl('core.setting', 'updateSetting', 'view', 'html').'&id='. Dispatcher::Instance()->Encrypt ($val['id']));
                
                $this->mrTemplate->clearTemplate('button_delete');
                $this->mrTemplate->addVar('button_delete', 'NAME', $val['name']);
                $this->mrTemplate->addVar('button_delete', 'URL', GtfwDispt()->GetUrl('core.setting', 'deleteSetting', 'do', 'json').'&id='. Dispatcher::Instance()->Encrypt ($val['id']));
        ...
        $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('core.setting', 'addSetting', 'view', 'html'));
   	}
}
?>