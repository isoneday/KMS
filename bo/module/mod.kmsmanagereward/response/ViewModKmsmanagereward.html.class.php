...
class ViewModKmsmanagereward extends HtmlResponse
{
   	function TemplateModule()
   	{
      	$this->SetTemplateBasedir(Configuration::Instance()->GetValue('application','docroot').'module/'.GtfwDispt()->mModule.'/template');
      	$this->SetTemplateFile('view_modkmsmanagereward.html');
   	}
   	...
   	{	
		...
		$total = $ObjModKmsmanagereward->countModKmsmanagereward($filter);
		$data = $ObjModKmsmanagereward->getModKmsmanagereward($filter);
   	...
   	function ParseTemplate($rdata = NULL)
   	{
        ...
        if (!empty($data) AND count($data)>0) {
            ...
            foreach ($data as $val) {       
                $val['no'] = $no;
                $val['url_detail'] = GtfwDispt()->GetUrl('mod.kmsmanagereward', 'detailModKmsmanagereward', 'view', 'html').'&id='.$val['reward'];
                
                $this->mrTemplate->clearTemplate('button_update');
                $this->mrTemplate->addVar('button_update', 'URL', GtfwDispt()->GetUrl('mod.kmsmanagereward', 'updateModKmsmanagereward', 'view', 'html').'&id='.$val['reward']);
                
                $this->mrTemplate->clearTemplate('button_delete');
                $this->mrTemplate->addVar('button_delete', 'NAME', $val['name']);
                $this->mrTemplate->addVar('button_delete', 'URL', GtfwDispt()->GetUrl('mod.kmsmanagereward', 'deleteModKmsmanagereward', 'do', 'json').'&id='.$val['reward']);
        ...
        $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('mod.kmsmanagereward', 'addModKmsmanagereward', 'view', 'html'));
   	}
}
?>