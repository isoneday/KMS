...
class ViewModKmsdata extends HtmlResponse
{
    ...
    if (!empty($filter)) {
        ...
        if (!empty($data) AND count($data)>0) {
           $file_path = Configuration::Instance()->GetValue('application', 'aplikan_photo');
...
//            print_r($data);
        ...
        //     $this->mrTemplate->parseTemplate('item', 'a');
           ...
           $this->mrTemplate->addVar('item','FILE2', $val['filedata_enc']);
              ...
              $this->mrTemplate->addVar('button_detail', 'URL', GtfwDispt()->GetUrl('mod.kmsdata', 'detailModKmsdata', 'view', 'json').'&id='.$val['data']);
