...
class ViewSearchengine extends HtmlResponse
{
   	function TemplateModule()
   	{
      	$this->SetTemplateBasedir(Configuration::Instance()->GetValue('application','docroot').'module/'.GtfwDispt()->mModule.'/template');
      	$this->SetTemplateFile('view_searchengine.html');
   	}
   	...
   	{	
		...
		$total = $ObjSearchengine->countSearchengine($filter);
		$data = $ObjSearchengine->getSearchengine($filter);
   	...
   	function ParseTemplate($rdata = NULL)
   	{
        ...
        $this->mrTemplate->addVar('search', 'URL', GtfwDispt()->GetUrl('searchengine', 'searchEnginedetail', 'view', 'html').'&display');
		if (!empty($filter)) {
            $this->mrTemplate->addVars('search', $filter);
        }
       
        
        $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('searchengine', 'addSearchengine', 'view', 'html'));
   	}
}
?>