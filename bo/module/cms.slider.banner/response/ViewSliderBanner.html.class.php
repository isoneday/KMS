 ...
 * @author Prima Noor 
 */
class ViewSliderBanner extends HtmlResponse {

    function TemplateModule() {
        $this->SetTemplateBasedir(Configuration::Instance()->GetValue('application', 'docroot') . 'module/' . GtfwDispt()->mModule . '/template');
        $this->SetTemplateFile('view_sliderbanner.html');
    }
    ...
    function ProcessRequest() {
        ...
        $total = $ObjSliderBanner->countSliderBanner($filter);
        $data = $ObjSliderBanner->getSliderBanner($filter);
    ...
    function ParseTemplate($rdata = NULL) {
        ...
        if (!empty($data) AND count($data) > 0) {
            ...
            foreach ($data as $val) {
                $val['no'] = $no;
                $val['url_detail'] = GtfwDispt()->GetUrl('cms.slider.banner', 'detailSliderBanner', 'view', 'html') . '&id=' . $val['id'];
                
                $path = Configuration::Instance()->GetValue('application', 'cms_save_path') . $val['photo'];
                
                $val['photo_file'] = "<b><a href=" . $path . " target=_blank> " . $val['photo_origin'] . "</a></b>";
                ...
                $this->mrTemplate->addVar('button_update', 'URL', GtfwDispt()->GetUrl('cms.slider.banner', 'updateSliderBanner', 'view', 'html') . '&id=' . $val['id']);

                $this->mrTemplate->clearTemplate('button_delete');
                $this->mrTemplate->addVar('button_delete', 'NAME', $val['title']);
                $this->mrTemplate->addVar('button_delete', 'URL', GtfwDispt()->GetUrl('cms.slider.banner', 'deleteSliderBanner', 'do', 'json') . '&id=' . $val['id']);
        ...
        $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('cms.slider.banner', 'addSliderBanner', 'view', 'html'));
    }
