...
class ViewComboModMeeting extends HtmlResponse
{
    function TemplateModule()
    {
        $this->SetTemplateBasedir(Configuration::Instance()->GetValue('application', 'docroot') . 'module/mod.meeting/template');
        $this->SetTemplateFile('view_combo_modmeeting.html');
    }
    ...
    function ProcessRequest()
    {
        ...
        Messenger::Instance()->Send('mod.meeting', 'comboModMeeting', 'view', 'html', $comboData, Messenger::UntilFetched);
        
        $allowAdd = Security::Authorization()->IsAllowedToAccess('mod.meeting', 'addModMeeting', 'view', 'html');
        
        $list = $ObjModMeeting->listModMeeting();
    ...
    function ParseTemplate($rdata = null)
    {
        ...
        if (!empty($comboData['elmId'])) {
            ...
            if (!empty($comboData['showAdd']) and $allowAdd) {
                ...
                $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('mod.meeting', 'addModMeeting', 'view', 'html').'&elmId='.$comboData['elmId']);
            }
        }
