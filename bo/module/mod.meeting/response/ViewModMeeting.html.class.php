...
class ViewModMeeting extends HtmlResponse
{
   	function TemplateModule()
   	{
      	$this->SetTemplateBasedir(Configuration::Instance()->GetValue('application','docroot').'module/'.GtfwDispt()->mModule.'/template');
      	$this->SetTemplateFile('view_modmeeting.html');
   	}
   	...
   	{	
		...
		$total = $ObjModMeeting->countModMeeting($filter);
		$data = $ObjModMeeting->getModMeeting($filter);
   	...
   	function ParseTemplate($rdata = NULL)
   	{
        ...
        if (!empty($data) AND count($data)>0) {
            ...
            foreach ($data as $val) {       
                $val['no'] = $no;
                $val['url_detail'] = GtfwDispt()->GetUrl('mod.meeting', 'detailModMeeting', 'view', 'html').'&id='.$val['id'];
                
                $this->mrTemplate->clearTemplate('button_update');
                $this->mrTemplate->addVar('button_update', 'URL', GtfwDispt()->GetUrl('mod.meeting', 'updateModMeeting', 'view', 'html').'&id='.$val['id']);
                
                $this->mrTemplate->clearTemplate('button_delete');
                $this->mrTemplate->addVar('button_delete', 'NAME', $val['name']);
                $this->mrTemplate->addVar('button_delete', 'URL', GtfwDispt()->GetUrl('mod.meeting', 'deleteModMeeting', 'do', 'json').'&id='.$val['id']);
        ...
        $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('mod.meeting', 'addModMeeting', 'view', 'html'));
   	}
}
?>