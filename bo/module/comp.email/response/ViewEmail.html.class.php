 ...
 * @author Prima Noor 
 */
class ViewEmailTemplate extends HtmlResponse {

    function TemplateModule() {
        $this->SetTemplateBasedir(Configuration::Instance()->GetValue('application', 'docroot') . 'module/' . GtfwDispt()->mModule . '/template');
        $this->SetTemplateFile('view_emailtemplate.html');
    }
    ...
    function ProcessRequest() {
        ...
        $data = $ObjEmailTemplate->getEmailTemplate($filter);
        $total = $ObjEmailTemplate->countEmailTemplate();
    ...
    function ParseTemplate($rdata = NULL) {
        ...
        if (!empty($data) AND count($data) > 0) {
            ...
            foreach ($data as $val) {
                $val['no'] = $no;
                $val['url_detail'] = GtfwDispt()->GetUrl('comp.email.template', 'detailEmailTemplate', 'view', 'html') . '&id=' . $val['id'];

                $this->mrTemplate->clearTemplate('button_update');
                $this->mrTemplate->addVar('button_update', 'URL', GtfwDispt()->GetUrl('comp.email.template', 'updateEmailTemplate', 'view', 'html') . '&id=' . $val['id']);

                $this->mrTemplate->clearTemplate('button_delete');
                $this->mrTemplate->addVar('button_delete', 'NAME', $val['subject']);
                $this->mrTemplate->addVar('button_delete', 'URL', GtfwDispt()->GetUrl('comp.email.template', 'deleteEmailTemplate', 'do', 'json') . '&id=' . $val['id']);
        ...
        $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('comp.email.template', 'addEmailTemplate', 'view', 'html'));
    }
