...
class ViewComboModListsearching extends HtmlResponse
{
    function TemplateModule()
    {
        $this->SetTemplateBasedir(Configuration::Instance()->GetValue('application', 'docroot') . 'module/mod.listsearching/template');
        $this->SetTemplateFile('view_combo_modlistsearching.html');
    }
    ...
    function ProcessRequest()
    {
        ...
        Messenger::Instance()->Send('mod.listsearching', 'comboModListsearching', 'view', 'html', $comboData, Messenger::UntilFetched);
        
        $allowAdd = Security::Authorization()->IsAllowedToAccess('mod.listsearching', 'addModListsearching', 'view', 'html');
        
        $list = $ObjModListsearching->listModListsearching();
    ...
    function ParseTemplate($rdata = null)
    {
        ...
        if (!empty($comboData['elmId'])) {
            ...
            if (!empty($comboData['showAdd']) and $allowAdd) {
                ...
                $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('mod.listsearching', 'addModListsearching', 'view', 'html').'&elmId='.$comboData['elmId']);
            }
        }
