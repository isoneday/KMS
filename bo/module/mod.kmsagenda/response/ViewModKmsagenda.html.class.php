...
class ViewModKmsagenda extends HtmlResponse
{
   	function TemplateModule()
   	{
      	$this->SetTemplateBasedir(Configuration::Instance()->GetValue('application','docroot').'module/'.GtfwDispt()->mModule.'/template');
      	$this->SetTemplateFile('view_modkmsagenda.html');
   	}
   	...
   	{	
		...
		$total = $ObjModKmsagenda->countModKmsagenda($filter);
		$data = $ObjModKmsagenda->getModKmsagenda($filter);
   	...
   	function ParseTemplate($rdata = NULL)
   	{
        ...
        if (!empty($data) AND count($data)>0) {
            ...
            foreach ($data as $val) {       
                $val['no'] = $no;
                $val['url_detail'] = GtfwDispt()->GetUrl('mod.kmsagenda', 'detailModKmsagenda', 'view', 'html').'&id='.$val['agenda'];
                
                $this->mrTemplate->clearTemplate('button_update');
                $this->mrTemplate->addVar('button_update', 'URL', GtfwDispt()->GetUrl('mod.kmsagenda', 'updateModKmsagenda', 'view', 'html').'&id='.$val['agenda']);
                
                $this->mrTemplate->clearTemplate('button_delete');
                $this->mrTemplate->addVar('button_delete', 'NAME', $val['name']);
                $this->mrTemplate->addVar('button_delete', 'URL', GtfwDispt()->GetUrl('mod.kmsagenda', 'deleteModKmsagenda', 'do', 'json').'&id='.$val['agenda']);
        ...
        $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('mod.kmsagenda', 'addModKmsagenda', 'view', 'html'));
   	}
}
?>