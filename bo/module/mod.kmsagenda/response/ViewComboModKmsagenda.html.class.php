...
class ViewComboModKmsagenda extends HtmlResponse
{
    function TemplateModule()
    {
        $this->SetTemplateBasedir(Configuration::Instance()->GetValue('application', 'docroot') . 'module/mod.kmsagenda/template');
        $this->SetTemplateFile('view_combo_modkmsagenda.html');
    }
    ...
    function ProcessRequest()
    {
        ...
        Messenger::Instance()->Send('mod.kmsagenda', 'comboModKmsagenda', 'view', 'html', $comboData, Messenger::UntilFetched);
        
        $allowAdd = Security::Authorization()->IsAllowedToAccess('mod.kmsagenda', 'addModKmsagenda', 'view', 'html');
        
        $list = $ObjModKmsagenda->listModKmsagenda();
    ...
    function ParseTemplate($rdata = null)
    {
        ...
        if (!empty($comboData['elmId'])) {
            ...
            if (!empty($comboData['showAdd']) and $allowAdd) {
                ...
                $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('mod.kmsagenda', 'addModKmsagenda', 'view', 'html').'&elmId='.$comboData['elmId']);
            }
        }
