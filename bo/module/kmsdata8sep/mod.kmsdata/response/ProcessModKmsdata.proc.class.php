...
//       move_uploaded_file($_FILES['lam_filedata']['tmp_name'], 'C:/xampp/htdocs/gtfw36cop/files/applicant/registulang/fileupload/'.$_FILES['lam_filedata']['name']);
    ...
    //}filedata_enc
        ...
        if ($upload_file != null) {
            if ($upload_file['lam_filedata']['error'] === false) {
                ...
                $post['lam_filedata_file_size']     = $upload_fil e['lam_filedata']['file_size'];
            }
        }        
    ...
    if (!empty($eksfile)) {
          if($eksfile=="docx"){
                ...
                $cmd=shell_exec('C:\\xampp\\htdocs\\gtkms\\files\\applicant\\registulang\\fileupload\\docx2txt.sh "'.$gab.'" ');
              //  echo $cmd; 
            } else if($eksfile=="pdf"){
            ...
            $gab2=$post['lam_filedata_file_path'].substr($post['lam_filedata_file'],0,-4).".txt";
               // $dsd =`>`;
         $cmd= shell_exec('C:\\xampp\\htdocs\\gtkms\\files\\applicant\\registulang\\fileupload\\antiword\\antiword.exe -f "'.$gab.'" > "'.$gab2.'"');
      ...
      //    else if($eksfile=="ppt"or$eksfile=="pptx"){
      // {    $gab4=$post['lam_filedata_file_path'].$post['lam_filedata_file'];

      // $cmd4=shell_exec('C:\\xampp\\htdocs\\gtfw36cop\\files\\applicant\\registulang\\fileupload\\pptx2pdf.vbs "'.$gab4.'" ');  
      //  }
      //  $x=$post['lam_filedata_file_txt'];
      //  $y   =explode(".",$x);
      //     $gabo=$post['lam_filedata_file_path'].$post['lam_filedata_file_txt'];
       
      //  if ($y=="pdf"){
      //   $cmd3=shell_exec('C:\\xampp\\htdocs\\gtfw36cop\\files\\applicant\\registulang\\fileupload\\pdftotext.exe "'.$gabo.'" '); 
      //      }
