...
class ViewComboModKmsdata extends HtmlResponse
{
    function TemplateModule()
    {
        $this->SetTemplateBasedir(Configuration::Instance()->GetValue('application', 'docroot') . 'module/mod.kmsdata/template');
        $this->SetTemplateFile('view_combo_modkmsdata.html');
    }
    ...
    function ProcessRequest()
    {
        ...
        Messenger::Instance()->Send('mod.kmsdata', 'comboModKmsdata', 'view', 'html', $comboData, Messenger::UntilFetched);
        
        $allowAdd = Security::Authorization()->IsAllowedToAccess('mod.kmsdata', 'addModKmsdata', 'view', 'html');
        
        $list = $ObjModKmsdata->listModKmsdata();
    ...
    function ParseTemplate($rdata = null)
    {
        ...
        if (!empty($comboData['elmId'])) {
            ...
            if (!empty($comboData['showAdd']) and $allowAdd) {
                ...
                $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('mod.kmsdata', 'addModKmsdata', 'view', 'html').'&elmId='.$comboData['elmId']);
            }
        }
