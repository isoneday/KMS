...
class ProcessModKmstraining
{
    ...
    {
        $this->ObjModKmstraining = GtfwDispt()->load->business('ModKmstraining');
    ...
    function input()
    {
        ...
        if ($result) {
            if (!$post['id']) {
                $this->ObjModKmstraining->StartTrans();
                $params = array(
                    $this->user,
                    $post['judul'],
                    $post['jenis'],
                ...
                $result = $result && $this->ObjModKmstraining->insertModKmstraining($params);
                $this->ObjModKmstraining->EndTrans($result);
                if ($result) {
                ...
                $result = $result && $this->ObjModKmstraining->updateModKmstraining($params);
                $this->ObjModKmstraining->EndTrans($result);   
                if ($result) {
        ...
        if ($result) {
            Messenger::Instance()->Send('mod.kmstraining', 'ModKmstraining', 'view', 'html', array(NULL, $msg, $css), Messenger::NextRequest);
            //return Dispatcher::Instance()->GetUrl('mod.kmstraining', 'modkmstraining', 'view', 'html');
        } else {
            Messenger::Instance()->Send('mod.kmstraining', 'inputModKmstraining', 'view', 'html', array($post, $msg, $css), Messenger::NextRequest);
            //return Dispatcher::Instance()->GetUrl('mod.kmstraining', (empty($post['id'])?'add':'update').'ModKmstraining', 'view', 'html');
        }
        return $result;     
    }
    ...
    function delete($id)
    {
        $result = $this->ObjModKmstraining->deleteModKmstraining($id);
        if ($result) {
