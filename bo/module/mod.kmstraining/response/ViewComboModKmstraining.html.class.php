...
class ViewComboModKmstraining extends HtmlResponse
{
    function TemplateModule()
    {
        $this->SetTemplateBasedir(Configuration::Instance()->GetValue('application', 'docroot') . 'module/mod.kmstraining/template');
        $this->SetTemplateFile('view_combo_modkmstraining.html');
    }
    ...
    function ProcessRequest()
    {
        ...
        Messenger::Instance()->Send('mod.kmstraining', 'comboModKmstraining', 'view', 'html', $comboData, Messenger::UntilFetched);
        
        $allowAdd = Security::Authorization()->IsAllowedToAccess('mod.kmstraining', 'addModKmstraining', 'view', 'html');
        
        $list = $ObjModKmstraining->listModKmstraining();
    ...
    function ParseTemplate($rdata = null)
    {
        ...
        if (!empty($comboData['elmId'])) {
            ...
            if (!empty($comboData['showAdd']) and $allowAdd) {
                ...
                $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('mod.kmstraining', 'addModKmstraining', 'view', 'html').'&elmId='.$comboData['elmId']);
            }
        }
