...
class ViewGroup extends HtmlResponse {
    ...
    function ProcessRequest() {
        ...
        $data = $ObjGroup->getGroup($filter);
        $total = $ObjGroup->countGroup();
    ...
    function ParseTemplate($rdata = NULL) {
        ...
        if (!empty($data) AND count($data) > 0) {
            ...
            foreach ($data as $val) {
                ...
                $val['url_detail'] = GtfwDispt()->GetUrl('core.group', 'detailGroup', 'view', 'html') . '&id=' . $val['id'];

                $this->mrTemplate->clearTemplate('button_update');
                $this->mrTemplate->addVar('button_update', 'URL', GtfwDispt()->GetUrl('core.group', 'updateGroup', 'view', 'html') . '&id=' . $val['id']);

                $this->mrTemplate->clearTemplate('button_delete');
                $url_delete = Dispatcher::Instance()->GetUrl('core.group', 'deleteGroup', 'do', 'json') . '&id=' . $val['id'];
                $this->mrTemplate->addVar('button_delete', 'URL', $url_delete);
        ...
        $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('core.group', 'addGroup', 'view', 'html'));
    }
