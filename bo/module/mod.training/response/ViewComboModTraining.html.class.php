...
class ViewComboModTraining extends HtmlResponse
{
    function TemplateModule()
    {
        $this->SetTemplateBasedir(Configuration::Instance()->GetValue('application', 'docroot') . 'module/mod.training/template');
        $this->SetTemplateFile('view_combo_modtraining.html');
    }
    ...
    function ProcessRequest()
    {
        ...
        Messenger::Instance()->Send('mod.training', 'comboModTraining', 'view', 'html', $comboData, Messenger::UntilFetched);
        
        $allowAdd = Security::Authorization()->IsAllowedToAccess('mod.training', 'addModTraining', 'view', 'html');
        
        $list = $ObjModTraining->listModTraining();
    ...
    function ParseTemplate($rdata = null)
    {
        ...
        if (!empty($comboData['elmId'])) {
            ...
            if (!empty($comboData['showAdd']) and $allowAdd) {
                ...
                $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('mod.training', 'addModTraining', 'view', 'html').'&elmId='.$comboData['elmId']);
            }
        }
