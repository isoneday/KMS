...
//echo pptx_to_text("C:\Users\Iswandi_Saputra\Downloads\pptx to txt\pptx2txt\pptx2txt-0.1\aha.pptx");
    ...
    function input()
    {
      ...
      //  $file_txt = Configuration::Instance()->GetValue('application', 'aplikan_filetxt');
        ...
        $config = array(
            "dest" => $file_path,
            "ext" => "pptx,txt,doc,DOC,docx,DOCX,pdf,PDF,jpg,jpeg,JPG,JPEG,png,PNG",  
            "maxsize" => 12097152
        );
...
//        echo "<pre>";print_r($_POST);echo "</pre>";die();
    ...
    if (!empty($eksfile)) {
          if($eksfile=="docx"){
                ...
                $cmd=shell_exec('C:\\xampp\\htdocs\\gtkms\\bo\\files\\applicant\\registulang\\fileupload\\docx2txt\\docx2txt.sh "'.$gab.'" ');
              //  echo $cmd; 
            } 
            else if($eksfile=="pdf")
            {
              $gabb=$post['lam_filedata_file_path'].$post['lam_filedata_file']; 
              $cmd=shell_exec('C:\\xampp\\htdocs\\gtkms\\bo\\files\\applicant\\registulang\\fileupload\\pdf2txt\\pdftotext.exe "'.$gabb.'"'); 
              // echo $cmd; 
             }
        else if($eksfile=="doc")
            {
            ...
            $gab2=$post['lam_filedata_file_path'].substr($post['lam_filedata_file'],0,-4).".txt";
               // $dsd =`>`;
            $cmd= shell_exec('C:\\xampp\\htdocs\\gtkms\\bo\\files\\applicant\\registulang\\fileupload\\antiword1\\antiword.exe -f "'.$gab.'" > "'.$gab2.'"');   
            }    
      else if($eksfile=="xlsx"or$eksfile=="xls")
            {
           $gab3=$post['lam_filedata_file_path'].$post['lam_filedata_file'];
           $cmd=shell_exec('C:\\xampp\\htdocs\\gtkms\\bo\\files\\applicant\\registulang\\fileupload\\xlstocsv\\XlsToCsv.vbs "'.$gab3.'" ');
           }
           else if($eksfile=="pptx")
           {
           $gabu=$post['lam_filedata_file_path'].$post['lam_filedata_file'];
           $gal=$post['lam_filedata_file'];
           $cmd=shell_exec('C:\\xampp\\htdocs\\gtkms\\bo\\files\\applicant\\registulang\\fileupload\\pptx2txtt\\pptx2txt.sh "'.$gabu.'" ');
...
//echo "<pre>".$this->GetLastError();
    ...
    function delete($id)
    {
       // echo "a";
        ...
        $getNameFile2 = $this->ObjModKmsdata->getNameFileById2($id);
        
        $file = $file_path.$getNameFile;
        $file2 = $file_path.$getNameFile2;
