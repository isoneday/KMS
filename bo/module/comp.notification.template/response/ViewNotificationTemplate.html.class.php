 ...
 * @author Prima Noor 
 */
class ViewNotificationTemplate extends HtmlResponse {

    function TemplateModule() {
        $this->SetTemplateBasedir(Configuration::Instance()->GetValue('application', 'docroot') . 'module/' . GtfwDispt()->mModule . '/template');
        $this->SetTemplateFile('view_notificationtemplate.html');
    }
    ...
    function ProcessRequest() {
        ...
        $data = $ObjNotificationTemplate->getNotificationTemplate($filter);
        $total = $ObjNotificationTemplate->countNotificationTemplate();
    ...
    function ParseTemplate($rdata = NULL) {
        ...
        if (!empty($data) AND count($data) > 0) {
            ...
            foreach ($data as $val) {
                $val['no'] = $no;
                $val['url_detail'] = GtfwDispt()->GetUrl('comp.notification.template', 'detailNotificationTemplate', 'view', 'html') . '&id=' . $val['id'];

                $this->mrTemplate->clearTemplate('button_update');
                $this->mrTemplate->addVar('button_update', 'URL', GtfwDispt()->GetUrl('comp.notification.template', 'updateNotificationTemplate', 'view', 'html') . '&id=' . $val['id']);

                $this->mrTemplate->clearTemplate('button_delete');
                $this->mrTemplate->addVar('button_delete', 'NAME', $val['purpose']);
                $this->mrTemplate->addVar('button_delete', 'URL', GtfwDispt()->GetUrl('comp.notification.template', 'deleteNotificationTemplate', 'do', 'json') . '&id=' . $val['id']);
        ...
        $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('comp.notification.template', 'addNotificationTemplate', 'view', 'html'));
    }
