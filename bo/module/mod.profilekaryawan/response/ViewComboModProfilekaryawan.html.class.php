...
class ViewComboModProfilekaryawan extends HtmlResponse
{
    function TemplateModule()
    {
        $this->SetTemplateBasedir(Configuration::Instance()->GetValue('application', 'docroot') . 'module/mod.profilekaryawan/template');
        $this->SetTemplateFile('view_combo_modprofilekaryawan.html');
    }
    ...
    function ProcessRequest()
    {
        ...
        Messenger::Instance()->Send('mod.profilekaryawan', 'comboModProfilekaryawan', 'view', 'html', $comboData, Messenger::UntilFetched);
        
        $allowAdd = Security::Authorization()->IsAllowedToAccess('mod.profilekaryawan', 'addModProfilekaryawan', 'view', 'html');
        
        $list = $ObjModProfilekaryawan->listModProfilekaryawan();
    ...
    function ParseTemplate($rdata = null)
    {
        ...
        if (!empty($comboData['elmId'])) {
            ...
            if (!empty($comboData['showAdd']) and $allowAdd) {
                ...
                $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('mod.profilekaryawan', 'addModProfilekaryawan', 'view', 'html').'&elmId='.$comboData['elmId']);
            }
        }
