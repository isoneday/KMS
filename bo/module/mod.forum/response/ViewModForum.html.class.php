...
class ViewModForum extends HtmlResponse
{
   	function TemplateModule()
   	{
      	$this->SetTemplateBasedir(Configuration::Instance()->GetValue('application','docroot').'module/'.GtfwDispt()->mModule.'/template');
      	$this->SetTemplateFile('view_modforum.html');
   	}
   	...
   	{	
		...
		$total = $ObjModForum->countModForum($filter);
		$data = $ObjModForum->getModForum($filter);
   	...
   	function ParseTemplate($rdata = NULL)
   	{
        ...
        if (!empty($data) AND count($data)>0) {
            ...
            foreach ($data as $val) {       
                $val['no'] = $no;
                $val['url_detail'] = GtfwDispt()->GetUrl('mod.forum', 'detailModForum', 'view', 'html').'&id='.$val['kategori_forum'];
                
                $this->mrTemplate->clearTemplate('button_update');
                $this->mrTemplate->addVar('button_update', 'URL', GtfwDispt()->GetUrl('mod.forum', 'updateModForum', 'view', 'html').'&id='.$val['kategori_forum']);
                
                $this->mrTemplate->clearTemplate('button_delete');
                $this->mrTemplate->addVar('button_delete', 'NAME', $val['name']);
                $this->mrTemplate->addVar('button_delete', 'URL', GtfwDispt()->GetUrl('mod.forum', 'deleteModForum', 'do', 'json').'&id='.$val['kategori_forum']);
        ...
        $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('mod.forum', 'addModForum', 'view', 'html'));
   	}
}
?>