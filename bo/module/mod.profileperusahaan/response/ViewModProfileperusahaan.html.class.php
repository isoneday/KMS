...
class ViewModProfileperusahaan extends HtmlResponse
{
   	function TemplateModule()
   	{
      	$this->SetTemplateBasedir(Configuration::Instance()->GetValue('application','docroot').'module/'.GtfwDispt()->mModule.'/template');
      	$this->SetTemplateFile('view_modprofileperusahaan.html');
   	}
   	...
   	{	
		...
		$total = $ObjModProfileperusahaan->countModProfileperusahaan($filter);
		$data = $ObjModProfileperusahaan->getModProfileperusahaan($filter);
   	...
   	function ParseTemplate($rdata = NULL)
   	{
        ...
        if (!empty($data) AND count($data)>0) {
            ...
            foreach ($data as $val) {       
                $val['no'] = $no;
                $val['url_detail'] = GtfwDispt()->GetUrl('mod.profileperusahaan', 'detailModProfileperusahaan', 'view', 'html').'&id='.$val['id'];
                
                $this->mrTemplate->clearTemplate('button_update');
                $this->mrTemplate->addVar('button_update', 'URL', GtfwDispt()->GetUrl('mod.profileperusahaan', 'updateModProfileperusahaan', 'view', 'html').'&id='.$val['id']);
                
                $this->mrTemplate->clearTemplate('button_delete');
                $this->mrTemplate->addVar('button_delete', 'NAME', $val['name']);
                $this->mrTemplate->addVar('button_delete', 'URL', GtfwDispt()->GetUrl('mod.profileperusahaan', 'deleteModProfileperusahaan', 'do', 'json').'&id='.$val['id']);
        ...
        $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('mod.profileperusahaan', 'addModProfileperusahaan', 'view', 'html'));
   	}
}
?>