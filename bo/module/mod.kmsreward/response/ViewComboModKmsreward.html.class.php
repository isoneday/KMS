...
class ViewComboModKmsreward extends HtmlResponse
{
    function TemplateModule()
    {
        $this->SetTemplateBasedir(Configuration::Instance()->GetValue('application', 'docroot') . 'module/mod.kmsreward/template');
        $this->SetTemplateFile('view_combo_modkmsreward.html');
    }
    ...
    function ProcessRequest()
    {
        ...
        Messenger::Instance()->Send('mod.kmsreward', 'comboModKmsreward', 'view', 'html', $comboData, Messenger::UntilFetched);
        
        $allowAdd = Security::Authorization()->IsAllowedToAccess('mod.kmsreward', 'addModKmsreward', 'view', 'html');
        
        $list = $ObjModKmsreward->listModKmsreward();
    ...
    function ParseTemplate($rdata = null)
    {
        ...
        if (!empty($comboData['elmId'])) {
            ...
            if (!empty($comboData['showAdd']) and $allowAdd) {
                ...
                $this->mrTemplate->addVar('button_add', 'URL', GtfwDispt()->GetUrl('mod.kmsreward', 'addModKmsreward', 'view', 'html').'&elmId='.$comboData['elmId']);
            }
        }
